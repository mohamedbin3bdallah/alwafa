<!-- footer content -->
	...
	$(document).ready(function(){
		$(".bills").click(function() {
			var bill = $(this).attr('oid');
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url(); ?>orders/getbill',
				data: {
					'bill':bill
				},
				success: function (response) { document.getElementById('mybill-'+bill).innerHTML = response; $("#bill-"+bill).modal('show'); }
			});
		});
	});
	$(document).ready(function(){
		$(".wvs").click(function() {
			var order = $(this).attr('oid');
			$.ajax({
				type: 'POST',
				url: '<?php echo base_url(); ?>orders/getwithdrowvouchers',
				data: {
					'order':order
				},
				success: function (response) { document.getElementById('mywv-'+order).innerHTML = response; $("#wv-"+order).modal('show'); }
			});
		});
	});
	</script>
